...
const storage = new Storage({
  keyFilename: path.join(
    ...
    "/peopleslens-pbl-firebase-adminsdk-pohc3-6f89177c29.json",
  ),
});
...
app.use(cors({ origin: "https://peoples-lens-production.up.railway.app/" }));
...
app.get("/user_information/userInfo.html", (req, res) => {
  res.sendFile(
    path.join(path.resolve(__dirname, "../")),
    "/user_information/userInfo.html",
  );
});
...
app.get("/capture_person/capturePerson.html", (req, res) => {
  res.sendFile(
    path.join(path.resolve(__dirname, "../")),
    "/capture_person/capturePerson.html",
  );
});
...
app.get("/recognize_user/recognizeUser.html", (req, res) => {
  res.sendFile(
    path.join(path.resolve(__dirname, "../")),
    "/recognize_user/recognizeUser.html",
  );
});
...
app.get("/display_information/displayInformation.html", (req, res) => {
  res.sendFile(
    path.join(path.resolve(__dirname, "../")),
    "/display_information/displayInformation.html",
  );
});
...
app.get("/recognize_user/sendInfo", async (req, res) => {
  try {
    ...
    const result = await pool.query(
      "SELECT id, name, domain FROM user_information",
    );
    if (result.rows.length === 0) {
      return res.json([]);
    }
...
app.post("/login/userLogin.html", async (req, res) => {
  try {
    ...
    if (dbres.rows.length !== 0) {
      const boolPass = await bcrypt.compare(
        req.body["user-pswd"],
...
function createPool() {
  const conString = process.env.DATABASE_PRIVATE_URL;
  return new Pool({
